...
abstract class SocialUserProvider implements UserProviderInterface
{
  ...
  public function __construct( Validator $validator, UserManager $userManager, SocialUserControllerService $socialUserManager )
  {
  ...
  public function loadUserByUsername( $username )
  {
    ...
    if ( !empty( $data ) )
    {
      ...
      $socialUser = $this->objectManager->getRepository( "BITSocialUserBundle:User" )->findOneBy( array( "social_id" => $data['id'] ) );
      ...
      if ( !is_object( $socialUser ) )
      {
        ...
        $user->addGroup( $this->objectManager->getRepository( "BITUserBundle:Group" )->findOneBy( array( "name" => strtoupper( $this->providerName ) ) ) );
  ...
  public function refreshUser( UserInterface $user )
  {
    ...
    if ( !is_object( $entity ) )
    {
      throw new UnsupportedUserException( sprintf( 'Instances of "%s" are not %s.', get_class( $user ), $this->providerName ));
    }
